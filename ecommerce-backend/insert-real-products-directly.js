...
async function insertRealProductsDirectly() {
  const client = new Client(renderDbConfig);
  
  try {
    console.log('🚀 直接插入真实商品数据到Render数据库...');
      ...
      try {
        ...
        try {
          ...
          if (result.rows.length > 0) {
            categoryMap[category.name] = result.rows[0].id;
            console.log(`   ✅ 使用现有主分类: ${category.name} (ID: ${result.rows[0].id})`);
          }
        } catch (selectError) {
          console.log(`   ⚠️ 主分类 ${category.name} 处理失败: ${error.message}`);
        }
      }
    }
    ...
    console.log(`📊 插入结果: 成功 ${successCount} 个商品，失败 ${failCount} 个`);
    console.log('💡 提示：');
