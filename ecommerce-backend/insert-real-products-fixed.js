...
// 直接通过SQL插入真实商品数据到Render数据库（修复版本）
...
async function insertRealProductsFixed() {
  const client = new Client(renderDbConfig);
  
  try {
    console.log('🚀 直接插入真实商品数据到Render数据库（修复版本）...');
    ...
    console.log(`   🛍️ 商品数量: ${productCount.rows[0].count}`);
    ...
    if (parseInt(categoryCount.rows[0].count) === 0) {
      ...
      const mainCategories = [
        { name: 'Electronics', description: 'Electronic devices and gadgets', sort_order: 1 },
        { name: 'Fashion', description: 'Clothing and accessories', sort_order: 2 },
        { name: 'Home & Garden', description: 'Home improvement and garden supplies', sort_order: 3 },
        { name: 'Sports & Outdoors', description: 'Sports equipment and outdoor gear', sort_order: 4 },
        { name: 'Beauty & Health', description: 'Beauty products and health supplements', sort_order: 5 },
        { name: 'Books & Media', description: 'Books, movies, and music', sort_order: 6 },
        { name: 'Toys & Games', description: 'Toys and gaming products', sort_order: 7 },
      ...
      for (const category of mainCategories) {
        ...
        console.log(`   ✅ 创建主分类: ${category.name} (ID: ${result.rows[0].id})`);
      }
    ...
    if (parseInt(productCount.rows[0].count) === 0) {
      ...
      // 获取分类ID映射
      const categoriesResult = await client.query('SELECT id, name FROM category ORDER BY id');
      const categoryMap = {};
      ...
      const realProducts = [
        // TechStore Malaysia 商品
        {
          ...
          categoryId: categoryMap['Smartphones'] || categoryMap['Electronics'] || 1,
          brand: 'Apple',
        ...
        },
        {
          ...
          categoryId: categoryMap['Laptops'] || categoryMap['Electronics'] || 1,
          brand: 'Apple',
        ...
        },
        {
          ...
          categoryId: categoryMap['Audio'] || categoryMap['Electronics'] || 1,
          brand: 'Apple',
        ...
        // Fashion Hub KL 商品
        {
          ...
          categoryId: categoryMap['Shoes'] || categoryMap['Fashion'] || 2,
          brand: 'Nike',
        ...
        },
        {
          ...
          categoryId: categoryMap['Men\'s Clothing'] || categoryMap['Fashion'] || 2,
          brand: 'Uniqlo',
        ...
        // Home Depot Malaysia 商品
        {
          ...
          categoryId: categoryMap['Furniture'] || categoryMap['Home & Garden'] || 3,
          brand: 'IKEA',
        ...
        },
        {
          ...
          categoryId: categoryMap['Kitchen & Dining'] || categoryMap['Home & Garden'] || 3,
          brand: 'KitchenAid',
      ...
      console.log(`📊 商品插入结果: 成功 ${successCount} 个，失败 ${failCount} 个`);
    } else {
      console.log('🛍️ 商品已存在，跳过创建');
    }
    ...
    // 检查分类数量
    const finalCategoryCount = await client.query('SELECT COUNT(*) as count FROM category');
    console.log(`   📂 分类数量: ${finalCategoryCount.rows[0].count}`);
    
    // 检查商品数量
    const finalProductCount = await client.query('SELECT COUNT(*) as count FROM platform_product');
    console.log(`   🛍️ 商品数量: ${finalProductCount.rows[0].count}`);
    
    // 检查商家数量
    const finalMerchantCount = await client.query('SELECT COUNT(*) as count FROM merchant');
    console.log(`   👥 商家数量: ${finalMerchantCount.rows[0].count}`);
    
    // 检查用户数量
    const finalUserCount = await client.query('SELECT COUNT(*) as count FROM "user"');
    console.log(`   👤 用户数量: ${finalUserCount.rows[0].count}`);
    
    console.log('✅ 真实商品数据插入完成！');
    console.log('💡 提示：');
    ...
    console.log('   5. 用户可以在前端看到这些商品');
    
  } catch (error) {
    console.error('❌ 真实商品数据插入失败:');
